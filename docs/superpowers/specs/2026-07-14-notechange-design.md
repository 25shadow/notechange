...
> 2026-07-14 范围更新：用户明确取消本地加密。当前导出产物为普通 JSON，迁移检查点仅保存在进程内；下文涉及加密任务缓存和系统凭据存储的旧设计不再执行。
...
3. 来源适配器分页读取文件夹和笔记摘要，获取完整笔记，并将图片附件下载到本地临时缓存。
...
- 导出 JSON 是明文文件，由用户自行保管；应用不额外复制账号凭据、Cookie 或认证头。
